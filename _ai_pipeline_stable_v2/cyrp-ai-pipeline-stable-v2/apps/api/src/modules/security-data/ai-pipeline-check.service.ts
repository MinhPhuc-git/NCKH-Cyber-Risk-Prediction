...
export class AiPipelineCheckService {
  ...
  async runForUserDevice(
    ...
    const running = this.activeChecks.get(deviceId);

    if (running) {
      this.logger.warn(
        `AI pipeline check already running for device=${deviceId}; returning the existing run instead of creating a conflict`,
      );
      return running;
    }
  ...
  private async ensureXgboostModel(): Promise<PipelineStepResult> {
    ...
    if (!existsSync(artifact)) {
      throw new ServiceUnavailableException({
        code: 'AI_MODEL_ARTIFACT_MISSING',
        message:
          'Không tìm thấy Model Result/xgboost/xgboost_model.pkl. Endpoint kiểm tra máy chỉ chạy dự đoán, không train lại model.',
        path: artifact,
      });
    }
    ...
    return {
      ...
      command: 'xgboost artifact already exists; skip training',
      skipped: true,
      durationMs: 0,
      stdoutTail: '',
      stderrTail: '',
