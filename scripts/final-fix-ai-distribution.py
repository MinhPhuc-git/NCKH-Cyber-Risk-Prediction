...
backup = path.with_suffix(".tsx.bak-final-ai-distribution-" + datetime.now().strftime("%Y%m%d-%H%M%S"))
...
replacement = '''async function loadAiRiskSummary() {
    ...
    function normalizeRiskLevel(value: unknown): keyof typeof distribution | null {
      ...
      if (level === 'CRITICAL' || level === 'VERY_HIGH' || level === 'VERY HIGH' || level === 'RẤT CAO' || level === 'RAT CAO') {
        return 'CRITICAL';
      }
    ...
    function toNumber(value: unknown): number | null {
      ...
      if (typeof value === 'string' && if (typeof value === 'number' && Number.isFinite(value)) return value;
...
if count == 0:
    raise SystemExit("Không tìm thấy block loadAiRiskSummary. Cần gửi đoạn quanh function loadAiRiskSummary.")
