...
export class AiPipelineDataUserImportService {
  ...
  async importDataUserJson(
    ...
    for (const file of files) {
      ...
      for (const record of records) {
        ...
        for (const agentId of agentIds) {
          ...
          try {
            ...
            fileImportedRecords += 1;
            result.recordsImported += 1;
            if (imported.vulnerabilityCreated) {
              result.vulnerabilitiesCreated += 1;
            }
      ...
      const canDeleteFile = options.deleteAfterImport !== false &&
        !fileHadFatalError &&
        fileImportedRecords > 0 &&
        fileSkippedRecords === 0;
  ...
  private async importOneRecord(
    ...
    vulnerabilityCreated: boolean;
    reason: string;
  }> {
    const resolvedDetection = await this.findOrCreateDetection(record, agentId, options);

    if (!resolvedDetection.detection) {
      return {
        imported: false,
        historyCreated: false,
        vulnerabilityCreated: false,
        reason: resolvedDetection.reason,
      };
    }

    const detection = resolvedDetection.detection;
    ...
    return {
      ...
      vulnerabilityCreated: resolvedDetection.created,
      reason: 'IMPORTED',
    };
  }

  private async findOrCreateDetection(
    record: DataUserPredictionRecord,
    agentId: string,
    options: DataUserImportOptions,
  ): Promise<{
    detection: (Prisma.DetectedVulnerabilityGetPayload<{
      include: {
        featureVector: {
    ...
    const existing = await this.database.detectedVulnerability.findFirst({
      where: {
        cveId: record.cveId,
    ...
    if (existing) {
      return { detection: existing, created: false, reason: 'FOUND_EXISTING' };
    }
    ...
    if (!agentId) {
      return {
        detection: null,
        created: false,
        reason: `Không có agent_id để map ${record.cveId} vào thiết bị CYRP`,
      };
    }

    const binding = await this.database.wazuhAgentBinding.findUnique({
      where: { wazuhAgentId: agentId },
      select: {
        wazuhAgentId: true,
        deviceId: true,
        device: {
    ...
    if (!binding) {
      return {
        detection: null,
        created: false,
        reason: `Không tìm thấy WazuhAgentBinding cho agent_id=${agentId}. Cần liên kết agent Wazuh với thiết bị CYRP trước.`,
      };
    }

    if (options.deviceId && binding.deviceId !== options.deviceId) {
      return {
        detection: null,
        created: false,
        reason: `agent_id=${agentId} đang liên kết với device_id=${binding.deviceId}, không khớp device_id=${options.deviceId}`,
      };
    }

    if (options.userId && binding.device.userId !== options.userId) {
      return {
        detection: null,
        created: false,
        reason: `agent_id=${agentId} không thuộc user hiện tại`,
      };
    }
    ...
    await this.database.cve.upsert({
      ...
      create: {
        cveId: record.cveId,
        description: this.optionalString(record.raw.Description) ??
          this.optionalString(record.raw.description) ??
          `Imported from AI Data User pipeline for ${record.cveId}`,
        source: 'AI_PIPELINE',
      },
      update: {},
    });
    ...
    const now = new Date();
    const sourceIndex = 'ai-pipeline-data-user';
    const sourceDocumentId = `${agentId}:${record.cveId}`.slice(0, 512);

    const detection = await this.database.detectedVulnerability.upsert({
      where: {
      ...
      create: {
        ...
        sourceStatus: 'AI_PIPELINE_IMPORTED',
        severity: this.normalizeRiskLevel(this.optionalString(record.raw.Severity) ?? this.optionalString(record.raw.Risk)),
        cvssBaseScore: this.optionalNumber(record.raw.Base_Score) ??
          this.optionalNumber(record.raw.base_score) ??
          this.optionalNumber(record.raw.CVSS_Base_Score),
        firstSeenAt: now,
        lastSeenAt: now,
        sourceIndex,
        sourceDocumentId,
        sourceUpdatedAt: now,
        rawPayload: this.sanitizeJson({
          source: 'DATA_USER_JSON_PIPELINE',
          importedBecauseDetectedVulnerabilityWasMissing: true,
          agentId,
          cveId: record.cveId,
          rawPrediction: record.raw,
      ...
      update: {
        ...
        rawPayload: this.sanitizeJson({
          source: 'DATA_USER_JSON_PIPELINE',
          refreshedFromDataUserImport: true,
          agentId,
          cveId: record.cveId,
          rawPrediction: record.raw,
        }) as Prisma.InputJsonValue,
    ...
    return { detection, created: true, reason: 'CREATED_DETECTED_VULNERABILITY' };
  }
